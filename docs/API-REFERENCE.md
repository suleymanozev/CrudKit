...
Applied to a manually written DTO record or class. Tells auto-registration which DTO to use for the target entity.
...
Implement `ICrudMapper` to combine all three interfaces. When registered via DI, reflection is fully bypassed.
...
### Auto Registration via `UseCrudKit()`

`UseCrudKit()` automatically scans all loaded assemblies for `[CrudEntity]`-decorated types and registers their endpoints. Entities already registered manually are skipped.
...
`UseCrudKit()` automatically scans all loaded assemblies for `[CrudEntity]`-decorated types and registers their CRUD endpoints. No code generation needed.

### How It Works

1. Module endpoints (`IModule.MapEndpoints`) run first
2. Assembly scan finds all `[CrudEntity]` types
3. Already-registered entities are skipped
4. `[CreateDtoFor]` / `[UpdateDtoFor]` DTOs are discovered automatically
5. Endpoints are registered with discovered DTOs or entity-as-DTO fallback
...
Annotate a manually written DTO with `[CreateDtoFor(typeof(TEntity))]` or `[UpdateDtoFor(typeof(TEntity))]`. Auto-registration discovers these attributes and wires up the correct endpoint overload.
